...
{
public:
  Aliens(Surface * psurface=NULL, Spaceship * space=NULL, ALaser ** alaser=NULL,
	 double x=0.0, double y=0.0, double dy=-2.0, int w=4, int h=10,
	 int r=160, int g=4, int b=69) 
    : psurface_(psurface),
      pspace_(space), //alienLaser_(alienLaser),
      ...
      mode_(0)
  {
    ...
    total_aliens++; // Increment total aliens
    aliens_rect_ = aliens_image_.getRect();
    
    aliens_rect_.x = p_[0]  = SPACER + (aliens_rect_.w + SPACER) * column_;
    aliens_rect_.y = p_[1]  = D_BTWN_RALIENS + SPACER + (aliens_rect_.h + SPACER) * row_;
  }
  ...
  void draw()
  {
    ...
    {
      set_Coordinates();
      psurface_->put_image(aliens_image_, aliens_rect_); // put image on surface
    }
  }
...
int Aliens:: total_aliens = 0;
Image Aliens::aliens_image_("images/galaxian/GalaxianAquaAlien.gif");
