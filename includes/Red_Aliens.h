...
{
public:
  RedAliens(Surface * psurface=NULL, Spaceship * space=NULL, ALaser ** alaser=NULL,
	    double x=0.0, double y=0.0, double dy=-2.0, int w=4, int h=10,
	    int r=160, int g=4, int b=69)
    : psurface_(psurface), pspace_(space), aliens_per_row(ALIENS_PER_ROW),
      alaser_(alaser), is_alive_(true),
      w_(w), h_(h),
      r_(r), g_(g), b_(b),
      p_(0.0, 0.0),
      v_(.5, 0.0),
	  ...
	  
  {
    ...
    column_ = total_aliens - row_ * aliens_per_row;
    
    total_aliens++;
    aliens_rect_ = red_aliens_image_.getRect();
    
    aliens_rect_.x = p_[0]  = SPACER + (aliens_rect_.w + SPACER) * column_;
    aliens_rect_.y = p_[1]  = D_BTWN_YALIENS + SPACER + (aliens_rect_.h + SPACER) * row_;
  }
  ...
  void draw()
  {
    ...
    {
      set_Coordinates();
      psurface_->put_image(red_aliens_image_, aliens_rect_); // put image on surface
    }
  }
...
private:  // Move to private when complete
  Surface * psurface_;
  Spaceship * pspace_;
  static Image red_aliens_image_;
  static Image dead_alien_;
...
int RedAliens:: total_aliens = 0;
Image RedAliens::red_aliens_image_("images/galaxian/GalaxianRedAlien.gif");
